...
describe("Tokenizer-types", () => {
  ...
  function getTokenizerWithData(testData: string, test: ITokenizerTest): Promise<ITokenizer> {
    const testPath = getResourcePath('tmp.dat');
    return fs.writeFile(testPath, Buffer.from(testData, 'binary')).then(() => {
  ...
  tokenizerTests.forEach(tokenizerType => {
    ...
    describe(tokenizerType.name, () => {
      ...
      it("should decode buffer", () => {
        ...
        return getTokenizerWithData("\x05peter", tokenizerType).then(rst => {
      ...
      it("should contain fileSize if constructed from file-read-stream", () => {
        ...
        // ToDo
        return tokenizerType.loadTokenizer('test1.dat').then(rst => {
          assert.equal(rst.fileSize, 16, " ReadStreamTokenizer.fileSize");
          // ToDo (rst as FileTokenizer).close();
        });
      });
      ...
      describe("Parsing binary numbers", () => {
        ...
        it("should decode signed 8-bit integer (INT8)", () => {
          ...
          return getTokenizerWithData("\x00\x7f\x80\xff\x81", tokenizerType).then(rst => {
        ...
        it("should decode signed 16-bit big-endian integer (INT16_BE)", () => {
          ...
          return getTokenizerWithData("\x0a\x1a\x00\x00\xff\xff\x80\x00", tokenizerType).then(rst => {
        ...
        it("should decode signed 24-bit big-endian integer (INT24_BE)", () => {
          ...
          return getTokenizerWithData("\x00\x00\x00\xff\xff\xff\x10\x00\xff\x80\x00\x00", tokenizerType).then(rst => {
        ...
        it("should decode signed 32-bit big-endian integer (INT32_BE)", () => {
          ...
          return getTokenizerWithData("\x00\x00\x00\x00\xff\xff\xff\xff\x00\x10\x00\xff\x80\x00\x00\x00", tokenizerType).then(rst => {
        ...
        it("should decode unsigned 8-bit integer (UINT8)", () => {

          return getTokenizerWithData("\x00\x1a\xff", tokenizerType).then(rst => {
        ...
        it("should decode unsigned mixed 16-bit big/little-endian integer", () => {
          ...
          return getTokenizerWithData("\x1a\x00\x1a\x00\x1a\x00\x1a\x00", tokenizerType).then(rst => {
        ...
        it("should decode signed 24-bit big/little-endian integer (UINT24_LE/INT24_BE)", () => {
          ...
          return getTokenizerWithData("\x1a\x1a\x00\x1a\x1a\x00\x1a\x1a\x00\x1a\x1a\x00", tokenizerType).then(rst => {
        ...
        it("should decode unsigned 32-bit little/big-endian integer (UINT32_LE/UINT32_BE)", () => {
          ...
          return getTokenizerWithData("\x1a\x00\x1a\x00\x1a\x00\x1a\x00\x1a\x00\x1a\x00\x1a\x00\x1a\x00", tokenizerType).then(rst => {
      ...
      it("Transparency", function() {
        ...
        return fs.writeFile(pathTestFile, buf).then(() => {
          ...
          return tokenizerType.loadTokenizer(testFile).then(rst => {
            let expected = 0;
      ...
      it("Handle peek token", () => {
        ...
        return tokenizerType.loadTokenizer('test1.dat').then(rst => {
          ...
          return peekOnData(rst);
        });

      });

      it("Overlapping peeks", () => {

        return getTokenizerWithData("\x01\x02\x03\x04\x05", tokenizerType).then(rst => {
          ...
          return rst.peekBuffer(peekBuffer, 0, 3) // Peek #1
            .then(len => {
              ...
              return rst.peekBuffer(peekBuffer, 0, 2); // Peek #4
            }).then(len => {
      ...
      it("number", () => {
        ...
        return tokenizerType.loadTokenizer('test3.dat').then(tokenizer => {
      ...
      it("should throw an Error if we reach EOF while peeking a number", () => {
        return tokenizerType.loadTokenizer('test3.dat').then(tokenizer => {
      ...
      it("should be able to handle multiple ignores", () => {
        return tokenizerType.loadTokenizer('test1.dat').then(tokenizer => {
          return tokenizer.readToken(Token.UINT32_LE)
            .then(value => {
              assert.ok(typeof value === "number");
              assert.equal(value, 0x001a001a, "UINT24_LE #1");
      ...
      it("should be able to ignore (skip)", () => {
      ...
      describe('End-Of-File exception behaviour', () => {
        ...
        it("readBuffer()", () => {
          ...
          return fs.stat(getResourcePath(testFile)).then(stat => {
            return stat.size;
          }).then(fileSize => {
            return tokenizerType.loadTokenizer(testFile).then(tokenizer => {
        ...
        it("should throw an EOF if we read to buffer", () => {
        ...
        it("should throw an EOF if we peek to buffer", () => {
        ...
        it("should not throw an EOF if we peek to buffer with maybeLess=true", () => {

          const buffer = Buffer.alloc(4);

          return getTokenizerWithData("\x89\x54\x40", tokenizerType).then(rst => {
            return rst.peekBuffer(buffer, 0, buffer.length, rst.position, true).then(len => {
